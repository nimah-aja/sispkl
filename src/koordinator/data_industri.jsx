...
import { Download, FileSpreadsheet, FileText, FileSearch, MapPin } from "lucide-react";
...
export default function IndustriPage() {
  ...
  // ===============================
  // USER DATA - SESUAI DENGAN DATAPESERTA (KOORDINATOR)
  // ===============================
  const user = {
  ...
  const validateIndustri = (data) => {
    ...
    if (!data.pic || data.pic.length < 2)
      errors.pic = `Kolom PIC Industri minimal 2 karakter. Tambahkan ${2 - (data.pic?.length || 0)} karakter lagi.`;
    if (!data.pic_telp || data.pic_telp.length < 10)
      errors.pic_telp = `Kolom PIC telp Industri minimal 10 karakter. Tambahkan ${10 - (data.pic_telp?.length || 0)} karakter lagi.`;
    return errors;
  };
  ...
  const detailFields = [
    ...
    { name: "pic", label: "Pembimbing", type: "text" },
    { name: "pic_telp", label: "No. Telepon Pembimbing", type: "text" },
    { name: "jurusan_nama", label: "Konsentrasi Keahlian", type: "text" },
    { name: "kuota_siswa", label: "Kuota Siswa", type: "text" },
