...
const handleGeneratePDF = () => {
  ...
  try {
    ...
    const colRekapWidth = 60; // Total untuk 4 sub-kolom
    const colKeteranganWidth = 48;
    const subColWidth = colRekapWidth / 4; // 15mm per sub-kolom
    ...
    doc.text("I", marginLeft + colNoWidth + colNamaWidth + subColWidth*2 + subColWidth/2, subHeaderTop + 3.5, { align: 'center' });
    
    // Sub-kolom A
    doc.rect(marginLeft + colNoWidth + colNamaWidth + subColWidth*3, subHeaderTop, subColWidth, 6);
    doc.text("A", marginLeft + colNoWidth + colNamaWidth + subColWidth*3 + subColWidth/2, subHeaderTop + 3.5, { align: 'center' });
    ...
    daftarSiswa.forEach((siswa, index) => {
      ...
      // Sub-kolom A (Alpa)
      doc.rect(marginLeft + colNoWidth + colNamaWidth + subColWidth*3, currentY, subColWidth, rowHeight);
      doc.text(siswa.alpa || "-", marginLeft + colNoWidth + colNamaWidth + subColWidth*3 + subColWidth/2, currentY + rowHeight/2 + 1, { align: 'center' });
      ...
      if ((index + 1) % 8 === 0 && (index + 1) < daftarSiswa.length) {
        ...
        doc.text("I", marginLeft + colNoWidth + colNamaWidth + subColWidth*2 + subColWidth/2, newSubHeaderTop + 3.5, { align: 'center' });
        doc.rect(marginLeft + colNoWidth + colNamaWidth + subColWidth*3, newSubHeaderTop, subColWidth, 6);
        doc.text("A", marginLeft + colNoWidth + colNamaWidth + subColWidth*3 + subColWidth/2, newSubHeaderTop + 3.5, { align: 'center' });
                  ...
                  <table className="w-full border-collapse border border-black mb-4">
                    <thead>
                      <tr className="bg-gray-100">
                        ...
                        <th className="border border-black p-2 text-center" colSpan="4">Rekap Kehadiran</th>
                    ...
                    <tbody>
                      {daftarSiswa.map((siswa, index) => (
                        <tr key={siswa.id}>
                          ...
                          <td className="border border-black p-2 text-center">{siswa.alpa || "-"}</td>
                    ...
                    <div key={siswa.id} className="mb-4 p-3 bg-white rounded border border-gray-200">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            <label className="block text-sm text-gray-600 mb-1">Hadir (H)</label>
                            <input
                              type="text"
                              value={siswa.hadir}
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={siswa.alpa}
                              onChange={(e) => handleSiswaChange(siswa.id, 'alpa', e.target.value)}
                              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
