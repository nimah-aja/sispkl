// src/pages/siswa/components/addKelompok.jsx
import React, { useState, useEffect } from "react";
import { X, Search, UserPlus, Check } from "lucide-react";
import { getAvailableMembers, createPklGroup } from "../../utils/services/siswa/group";

export default function BuatKelompokModal({ isOpen, onClose, onCreate }) {
  ...
  const handleSubmit = async () => {
    if (selectedAnggota.length === 0) return;

    setIsSubmitting(true);
    try {
      // Ambil hanya nama dari anggota terpilih
      ...
      console.log("Submitting payload:", payload); // Untuk debugging

      // Panggil API create group
      const response = await createPklGroup(payload);
      
      console.log("Group created successfully:", response); // Untuk debugging
      
      // Panggil callback onCreate dengan data yang diperlukan
      onCreate(selectedAnggota);
      ...
      // Tampilkan pesan error ke user (bisa ditambahkan toast notification)
      alert(error?.message || "Gagal membuat kelompok. Silakan coba lagi.");
    } finally {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Tambahkan teman-temanmu untuk membuat kelompok
            </p>
          </div>
        </div>
            ...
            <>
              ...
              {!searchTerm && selectedAnggota.length === 0 && (
                <div className="text-center py-8 text-gray-500">
                  <p>Ketik untuk mencari anggota</p>
                </div>
              )}
            </>
          )}
        ...
        <div className="flex items-center justify-end gap-3 p-6 border-t border-gray-200 bg-gray-50 flex-shrink-0">
          ...
          >
            {isSubmitting ? (
              ...
              <>
                <UserPlus size={18} />
                <span>Buat kelompok & buat undangan</span>
              </>
            )}
          </button>
        </div>
