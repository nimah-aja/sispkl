...
const KaproPindahPKL = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await getPindahPklKapro();
      ...
      const mapped = items.map((item) => {
        ...
        return {
          id: item.id,
          type,
          hasActions: item.status === "pending_kaprog",
  ...
  const handleDecision = async (mode, payload) => {
    try {
      await decidePindahPklKapro(detailData.id, {
  ...
  const exportExcel = () => {
    ...
    XLSX.writeFile(wb, "PindahPKL_Kapro.xlsx");
    setOpenExport(false);
  };
