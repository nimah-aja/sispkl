...
// Komponen CloudUpload untuk Edit
function CloudUploadEdit({ images, setImages, setAllUploaded }) {
        ...
        <div className="flex-1 flex flex-col min-h-0">
          ...
          <div className="grid grid-cols-2 gap-4 overflow-y-auto flex-1 pr-2">
            {images.map((img, index) => (
              <div key={img.id || index} className="bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow group">
                <div className="relative aspect-square bg-gray-100">
                  ...
                  <div className={`absolute top-2 left-2 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1
                    ${img.isExisting ? 'bg-blue-500' : 'bg-green-500'}`}
                  >
                    <CheckIcon className="w-3 h-3" /> 
                    {img.isExisting ? 'Lama' : 'Baru'}
                  </div>
                </div>
                ...
                <div className="p-3">
                  ...
                  <div className="flex justify-between items-center mt-2 text-xs text-gray-600">
                    <span>{formatFileSize(img.originalSize || 0)}</span>
...
export default function EditRealisasi() {
  ...
  const [industriNama, setIndustriNama] = useState("-");
  const [kegiatanNama, setKegiatanNama] = useState("-");

  const { realisasiId } = location.state || {};

  console.log("EditRealisasi - realisasiId:", realisasiId);

  // Fetch detail realisasi dan data tasks untuk mendapatkan nama industri & kegiatan
  useEffect(() => {
    const fetchData = async () => {
      if (!realisasiId) {
        toast.error("ID Realisasi tidak ditemukan");
        navigate(-1);
        return;
      }
      ...
      try {
        ...
        // Fetch detail realisasi
        const detail = await getRealisasiById(realisasiId);
        console.log("Detail realisasi:", detail);
        setRealisasiData(detail);
        
        // Fetch tasks untuk mendapatkan nama industri dan kegiatan
        const tasksRes = await getGuruTasks();
        const tasks = tasksRes.data || [];
        
        // Cari nama industri dan kegiatan berdasarkan ID
        for (const industriGroup of tasks) {
          if (industriGroup.industri?.id === detail.industri_id) {
            setIndustriNama(industriGroup.industri?.nama || "-");
            
            // Cari nama kegiatan
            const task = industriGroup.tasks.find(
              (t) => t.kegiatan?.id === detail.kegiatan_id
            );
            if (task) {
              setKegiatanNama(task.kegiatan?.jenis || "-");
            }
            break;
          }
        }
        ...
        if (detail.bukti_foto_urls?.length > 0) {
          const existingImages = detail.bukti_foto_urls.map((url, index) => ({
            url: url,
            id: `existing-${index}-${Date.now()}`,
            originalName: `Bukti ${index + 1}`,
            filename: `Bukti ${index + 1}`,
            originalSize: 0,
            preview: url,
            isExisting: true
          }));
    ...
    fetchData();
  }, [realisasiId, navigate]);

  // Fields untuk form edit - KOSONGKAN karena kita akan tampilkan di box terpisah
  const fields = [];
  ...
  useEffect(() => {
    ...
    if (hasImages && !allImagesUploaded) {
      ...
      toast.success("Semua gambar siap disimpan!", { id: "upload-status", duration: 3000 });
    } else if (!hasImages) {
  ...
  const handleSubmit = async () => {
    ...
    if (images.length === 0) {
      toast.error("Minimal 1 gambar bukti");
      return;
    }
    ...
    try {
      ...
      await updateRealisasiFoto(realisasiId, images.map(img => img.url));

      toast.success("Foto bukti berhasil diperbarui!", { id: toastId });
      ...
      const errorMessage = err.response?.data?.message || "Gagal memperbarui foto bukti";
  ...
  const handleCancel = () => {
    if (images.length > 0) {
      const confirmReset = window.confirm(
        `Yakin ingin membatalkan? ${images.length} gambar yang sudah diupload akan dihapus.`
      );
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-gray-600">Memuat data realisasi...</p>
        </div>
      </div>
    );
  }

  // Jika realisasiData masih null, tampilkan loading
  if (!realisasiData) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
  ...
  return (
    ...
    >
      {/* Box abu-abu untuk menampilkan data dari API */}
      <div className="p-4 bg-gray-50 rounded-lg mb-4">
        <h3 className="font-semibold text-gray-700 mb-3">Informasi Realisasi</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs text-gray-500">Nama Kegiatan</label>
            <p className="font-medium text-gray-800">{kegiatanNama}</p>
          </div>
          <div>
            <label className="text-xs text-gray-500">Nama Industri</label>
            <p className="font-medium text-gray-800">{industriNama}</p>
          </div>
          <div>
            <label className="text-xs text-gray-500">Tanggal Realisasi</label>
            <p className="font-medium text-gray-800">
              {dayjs(realisasiData.tanggal_realisasi).format("DD-MM-YYYY")}
            </p>
          </div>
          <div>
            <label className="text-xs text-gray-500">Status</label>
            <p className={`font-medium ${
              realisasiData.status === 'Sudah' ? 'text-green-600' : 'text-yellow-600'
            }`}>
              {realisasiData.status}
            </p>
          </div>
          <div className="md:col-span-2">
            <label className="text-xs text-gray-500">Catatan</label>
            <p className="font-medium text-gray-800">{realisasiData.catatan || '-'}</p>
          </div>
          <div className="md:col-span-2">
            <label className="text-xs text-gray-500">Dibuat Pada</label>
            <p className="font-medium text-gray-800">
              {dayjs(realisasiData.created_at).format("DD-MM-YYYY HH:mm")}
            </p>
          </div>
          <div className="md:col-span-2">
            <label className="text-xs text-gray-500">Diperbarui Pada</label>
            <p className="font-medium text-gray-800">
              {dayjs(realisasiData.updated_at).format("DD-MM-YYYY HH:mm")}
            </p>
          </div>
        </div>
      </div>
